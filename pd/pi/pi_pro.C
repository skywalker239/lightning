...
pi_t::_size_t pi_t::pro_t::size() const throw() {
	...
	switch(type) {
		...
		case _string:
			...
			return string_t::_size(str->size());

		case _array: {
			if(variant == 1) {
				_size_t size = 0;
				count_t count = array->count;

				for(pro_t const *_ptr = &array->items[count]; _ptr-- > array->items; )
					size += _ptr->size();
		...
		case _map: {
			if(variant == 1) {
				...
				for(
					map_t::item_t const *_ptr = &map->items[count];
					_ptr-- > map->items;
				) {
				...
				for(pair_t const *pair = pairs; pair; pair = pair->next) {
					++count;
...
pi_t::root_t *pi_t::build(pro_t const &pi_pro, mem_t &mem) {
